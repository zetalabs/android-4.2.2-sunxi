...
void Volume::deleteUnMountPoint(int clear){
    ...
    for (i = 0; i < MAX_UNMOUNT_PARTITIONS; i++) {
        if (mUnMountPart[i]) {
            ...
            snprintf(buf, sizeof(buf), "%s", mUnMountPart[i]);
...
int Volume::mountVol() {
    ...
    if (((mPartIdx == -1) &&(n > 1)) && mMountpoint) {
        ...
        if (Tmpfs::doMount(mMountpoint, AID_SYSTEM, AID_MEDIA_RW, 0702, true)) {
            SLOGE("%s failed to mount via tmpfs (%s)\n", mMountpoint, strerror(errno));
            return -1;
        }
    }
    ...
    for (i = 0; i < n; i++) {
        ...
        if ((mPartIdx == -1) && (n > 1)) {
            ...
            if (doMoveMount("/mnt/secure/staging", mMountPart[i], false)) {
                 ...
                 if (deleteMountPoint(mMountPart[i]) == 0) {
                     mMountPart[i] = NULL;
                 }
                 umount("/mnt/secure/staging");
                 continue;
    ...
    for (i=0;i<mMountedPartNum;i++)
    {
    	if (mPartIdx==-1&&mMountedPartNum>1) {
            if (mMountPart[i]) {
                ...
                snprintf(buf, sizeof(buf), "%s", mMountPart[i]);
